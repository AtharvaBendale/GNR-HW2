...
for epoch in range(params['num_epochs']):
    ...
    for i, (data, _) in tqdm(enumerate(dataloader, 0), total=data_len):
        # Get batch size
        b_size = data.size(0)
        ...
        loss_real = criterionD(probs_real, label.to(torch.float32))
        # Calculate gradients.
        loss_real.backward()
        ...
        loss_fake = criterionD(probs_fake, label.to(torch.float32))
        # Calculate gradients.
        loss_fake.backward()
        ...
        gen_loss = criterionD(probs_fake, label.to(torch.float32))
